...
export async function GET(
  ...
  if (!isOwner && !membership) {
    return NextResponse.json({ error: "You do not have access to this guest job." }, { status: 403 });
  }

  const ownBid = membership
    ? await prisma.guestAuctionBid.findUnique({
        where: {
  ...
  return NextResponse.json({
    ...
    auction: {
      ...
      bidCount: isOwner ? auction._count.bids : undefined,
      awardedBidId: isOwner ? auction.awardedBidId : undefined,
