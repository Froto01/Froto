    ...
    <main className="min-h-screen bg-gradient-to-b from-froto-ice via-slate-50 to-white pb-16">
      ...
      <div className="mx-auto max-w-6xl space-y-6 px-4 pt-7">
        ...
        <Card id="spot-offers" className="scroll-mt-6 rounded-[1.75rem] border-froto-teal/10 bg-white shadow-md shadow-froto-navy/5">
          ...
          <CardContent className="space-y-3">{spotOffers.length === 0 ? <p className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4 text-sm text-slate-500">No private spot offers submitted yet.</p> : spotOffers.map((offer) => <Link key={offer.id} href={`/platform/spot-requirements/${offer.spotRequirementId}`} className="block rounded-2xl border border-slate-200 bg-slate-50/60 p-4 transition hover:bg-teal-50/35"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><p className="font-semibold text-froto-navy">{offer.spotRequirement.title}</p><Badge className={offer.status === "AWARDED" ? "bg-froto-green text-white" : offer.status === "NOT_SELECTED" ? "bg-slate-500 text-white" : "bg-froto-teal text-white"}>{prettyStatus(offer.status)}</Badge></div><p className="mt-1 text-sm text-slate-500">Customer · {offer.spotRequirement.company.name}</p></div><div className="text-left sm:text-right"><p className="text-xs text-slate-500">Your private offer</p><p className="font-semibold text-froto-blue">{formatAUD(Number(offer.amount))}</p></div></div></Link>)}</CardContent>
        </Card>
