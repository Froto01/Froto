...
import { ArrowRight, Bell, Clock3, History, PackageCheck, Radar, Trophy } from "lucide-react";
...
export default async function GuestDashboardPage() {
  ...
  const [auctions, unreadNotificationCount] = await Promise.all([
    prisma.guestAuction.findMany({ where: { createdByUserId: user.id }, orderBy: { createdAt: "desc" }, include: { _count: { select: { bids: true } }, awardedBid: { include: { bidderCompany: { select: { name: true, verified: true } } } } } }),
    prisma.notification.count({ where: { companyId: null, recipientUserId: user.id, readAt: null } }),
  ]);
  ...
  return <main className="min-h-screen bg-gradient-to-b from-froto-ice via-slate-50 to-white pb-16"><div className="mx-auto max-w-5xl px-4 py-8">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-froto-blue">Guest dashboard</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-froto-navy">My transport requests</h1><p className="mt-2 max-w-2xl text-sm text-slate-500">A simple view of the jobs you have posted, company offers received and your awarded transport work.</p></div><div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link href="/platform">Marketplace</Link></Button><Button asChild variant="outline" className="relative"><Link href="/platform/notifications"><Bell className="mr-2 h-4 w-4" />Notifications{unreadNotificationCount > 0 ? <Badge className="ml-2 bg-froto-green text-white">{unreadNotificationCount}</Badge> : null}</Link></Button><Button asChild variant="outline"><Link href="/platform/opportunity-alerts"><Radar className="mr-2 h-4 w-4" />Opportunity alerts</Link></Button><Button asChild className="bg-froto-navy hover:bg-[#0a356f]"><Link href="/platform/guest-auctions">Post a job</Link></Button></div></div>
